...
from easy_shaders import SimpleModelViewProjectionShaderProgram, textureSimpleSetup, SimpleModelViewProjectionShaderProgramTex
...
class Camara:
    ...
    def moveDirection(self,direction):
        #funcion que rota la camara como pide el enunciado
        alpha = np.pi * 2 / 15
        if direction == "left":
            alpha = -alpha
        self.pos = tr.matmul([tr.rotationZ(alpha),self.pos])
...
def keyCallback(GLFWwindow,key,scancode,action,mods):
    if key == glfw.KEY_LEFT and action == glfw.PRESS:
        camara.moveDirection("left")
    elif key == glfw.KEY_RIGHT and action == glfw.PRESS:
        camara.moveDirection("right")
